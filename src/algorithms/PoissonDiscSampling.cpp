...
std::vector<glm::vec2> GeneratePoints(glm::vec2 grid_size, float radius,
                                      uint32_t numSamples) {
  ...
  while (active.size() > 0) {
    ...
    for (size_t n = 0; n < k; n++) {
      auto sample = glm::normalize(
          glm::vec2(generator->Float(-1.f, 1.f), generator->Float(-1.f, 1.f)));
